...
void BranchAndBound::calculatingPath(int startVert)
{
	...
	do
	{
		vector.clear();	//usuwam z wektora wszystkie struktury
...
bAndB BranchAndBound::matrixStartReduction(long** graph, int verticles, int startVert)
{
	...
	{
		long minValue = LONG_MAX;			//poczatkowa wartosc minimalna jest rowna maksymalnej wartosci "long int"
		for (int j = 0; j < verticles; j++)			//przechodzenie po wszystkich kolumnach
			if (i != j)			//mijanie przekatnej macierzy
			{
				...
				result.graph[i][j] = LONG_MAX;			//ustawiam "nieskonczonosc" na przekatnej macierzy
		for (int j = 0; j < verticles; j++)		//przechodzenie po wrzystkich kolumnach
			if (i != j)			//mijanie przekatnej macierzy
				if (minValue != LONG_MAX)	//sprawdzam, czy minimalna wartosc ulegla jakiejkolwiek zmianie
					if (result.graph[i][j] != LONG_MAX)		//sprawdzam, czy dana komórka nie by³a ju¿ wczesniej zmieniana na nieskonczonosc
	...
	for (int i = 0; i < verticles; i++)		//iteracja po kolumnach
	{
		...
		for (int j = 0; j < verticles; j++)		//przechodzenie po wrzystkich wierszach
			if (i != j)			//mijanie przekatnej macierzy
				if (minValue != LONG_MAX)		//sprawdzam, czy minimalna wartosc ulegla jakiejkolwiek zmianie
					if (result.graph[j][i] != LONG_MAX)		//sprawdzam, czy dana komórka nie by³a ju¿ wczesniej zmieniana na nieskonczonosc
...
long** BranchAndBound::copyGraph(long** graph, int size)
{
	long int **newGraph = new long int*[size];		//tworze nowy wskaznik na tablice wskaznikow
	for (int i = 0; i < size; i++)
		newGraph[i] = new long int[size];		//w kazdym wierszu tworze nowa tablice dynamiczna
...
bAndB BranchAndBound::reducing(bAndB given, int startVert, int endVert, int firstVertex)
{
	...
	for (int i = 0; i < graph.getVertices(); i++)
	{
		result.graph[startVert][i] = LONG_MAX;		//ustawiam wartoœci w wierszu wierzcholka z ktorego przechodze na nieskonczonosc
		result.graph[i][endVert] = LONG_MAX;		//ustawiam wartosci w kolumnie wierzcholka do ktorego przechodze na nieskonczonosc
	}
	result.graph[endVert][firstVertex] = LONG_MAX;	//ustawiam nieskonczonosc w komorce odpowiadajacej przejsciu z (wierzcholka do ktorego przechodze) do (wierzcholka startowego)
	...
	for (int i = 0; i < graph.getVertices(); i++)		//przechodzenie po wszystkich wierszach
	{
		...
		for (int j = 0; j < graph.getVertices(); j++)		//przechodzenie po wrzystkich kolumnach
			if (i != j)			//mijanie przekatnej macierzy
				if(minValue != LONG_MAX)		//sprawdzam czy minimalna waga ulegla zmianie
					if(result.graph[i][j] != LONG_MAX)		//sprawdzam, czy komorka nie jest juz rowna nieskonczonosci
						result.graph[i][j] -= minValue;		//zmniejszanie kazdej wartosci w wierszu o jego wartosc minimaln¹
		if(minValue != LONG_MAX)	//sprawdzam czy minimalna waga ulegla zmianie
	...
	for (int i = 0; i < graph.getVertices(); i++)		//iteracja po kolumnach
	{
		...
		for (int j = 0; j < graph.getVertices(); j++)		//przechodzenie po wrzystkich wierszach
			if (i != j)			//mijanie przekatnej macierzy
				if (minValue != LONG_MAX)		//sprawdzam czy minimalna waga ulegla zmianie
					if (result.graph[j][i] != LONG_MAX)		//sprawdzam czy wartosc komórki nie wynosi nieskonczonosc
						result.graph[j][i] -= minValue;		//zmniejszanie kazdej wartosci w wierszu o jego wartosc minimaln¹
		if (minValue != LONG_MAX)		//sprawdzam czy minimalna waga ulegla zmianie
			result.reduction += minValue;		//uaktualniam chwilowa redukcje
	}
